...
class Rating
{
    ...
    public function take($count = 3)
    {
        if ($this->get == self::ALL) {
            if ($this->type == self::STUDENT) {
                return $this->institute_students();
            }

            if ($this->type == self::TEACHER) {
                return $this->institute_teachers();
            }
        }

        if ($this->get == self::FACULTY) {
            if ($this->type == self::STUDENT) {
                return $this->faculty_students();
            }

            if ($this->type == self::TEACHER) {
                return $this->faculty_teachers();
            }
        }
    ...
    public function attached_students($count = Rating::ALL)
    {
        $students = $this->user->employee->students()->with([
            ...
            'user.articles' => fn($q) => $q->select('user_id', 'criteria_id', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'criteria_id'),
            'user.scholarships' => fn($q) => $q->select('user_id', 'criteria_id', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'criteria_id'),
            'user.inventions' => fn($q) => $q->select('user_id', 'criteria_id', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'criteria_id'),
            'user.startups' => fn($q) => $q->select('user_id', 'type', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'type'),
            'user.grand_economies' => fn($q) => $q->select('user_id', 'criteria_id', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'criteria_id'),
            'user.olympics' => fn($q) => $q->select('user_id', 'criteria_id', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'criteria_id'),
            'user.lang_certificates' => fn($q) => $q->select('user_id', 'lang', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id', 'lang'),
            'user.achievements' => fn($q) => $q->select('user_id', DB::raw('COUNT(*) as count'))
                ->groupBy('user_id'),
        ])->get();
