...
$(document).ready(function () {
    ...
    if(studentSelect.val()) {
        currentChatId = studentSelect.val();
        loadChatMessages(currentChatId);
    }

    // Function to load chat messages for the selected student
    function loadChatMessages(chatId) {
        $.ajax({
            url: "{{ route('chat.messages', ['chat' => ':chatId']) }}".replace(':chatId', chatId),
            method: "GET",
    ...
    chatForm.on('submit', function (e) {
        ...
        if (message !== '' && currentChatId) {
            $.ajax({
                url: "{{ route('chat.sendMessage', ['chat' => ':chatId']) }}".replace(':chatId', currentChatId),
                method: "POST",
                data: {
                    message: message,
                    _token: "{{ csrf_token() }}"
    ...
    studentSelect.on('change', function () {
        currentChatId = $(this).val();
        if (currentChatId) {
            loadChatMessages(currentChatId); // Load messages for the selected student
        }
    });
    ...
    $(document).on('click', '.delete-message', function () {
        ...
        if (confirm('Xabarni o\'chirmoqchimisiz?')) {
            $.ajax({
                url: "{{ route('chat.deleteMessage', ['chat'=>':chatId']) }}".replace(':chatId', currentChatId),
                method: "POST",
                data: {
                ...
                success: function (response) {
                    loadChatMessages(currentChatId); // Reload messages after deleting
                },
                error: function () {
    ...
    // Periodically reload chat messages to check for new ones
