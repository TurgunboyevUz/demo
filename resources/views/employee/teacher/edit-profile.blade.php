...
<div class="content-wrapper">
    ...
    <div class="card mt-3">
        <div class="card-body">
            <form id="profileForm" action="{{ route('employee.teacher.edit-profile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="faculty_id" id="faculty" class="form-control">
                            @if($user->employee->faculty_id == null)
                                <option value="" disabled selected>Fakultet tanlang</option>
                            @endif
                            
                            @foreach($faculties as $faculty)
                                <option value="{{ $faculty->id }}" {{ $user->employee->faculty_id == $faculty->id ? 'selected' : '' }}>{{ $faculty->name }}</option>
                            @endforeach
                        </select>
                    </div>
